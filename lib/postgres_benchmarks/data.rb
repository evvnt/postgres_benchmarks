module PostgresBenchmarks
  class Data
    ...
    def generate
      begin
        ...
        0.upto(count-1) do |index|
          god_array << {'total_clicks' => total_clicks[index], 'clicks_by_publisher' => clicks_by_publisher[index], 'clicks_by_day' => clicks_by_day[index]}
        end
        ...
        @db.conn.exec("INSERT INTO event_publisher_urls_jsonb (event_id, url_id, clicks_by_day) VALUES ('#{all_json_data}')")
      ensure
        @db.conn.close
      end
    end
    ...
    def all_json_data
      ...
      result = @db.conn.exec(json_select_sql_query)

      binding.pry
      # result.map { |res| res['event_id'], res['publisher_id'], res['url_id'], res['clicks_data'] }
